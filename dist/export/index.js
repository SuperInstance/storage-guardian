...
exports.exportSlack = exportSlack;
exports.exportMarkdown = exportMarkdown;
const utils_1 = require("../core/utils");
// ---------------------------------------------------------------------------
// JSON Export
// ---------------------------------------------------------------------------
function exportJson(report, alerts, opts) {
    ...
    const output = {
        generatedAt: (0, utils_1.formatDate)(report.generatedAt),
        source: report.source,
        summary: {
            totalEntries: report.totalEntries,
            totalBytes: report.totalBytes,
            totalBytesHuman: (0, utils_1.formatBytes)(report.totalBytes),
            uniqueBytes: report.uniqueBytes,
            uniqueBytesHuman: (0, utils_1.formatBytes)(report.uniqueBytes),
            wastedBytes: report.wastedBytes,
            wastedBytesHuman: (0, utils_1.formatBytes)(report.wastedBytes),
    ...
    if (includeDuplicates) {
        output.duplicateGroups = report.duplicateGroups.map((g) => ({
            ...
            wastedBytesHuman: (0, utils_1.formatBytes)(g.wastedBytes),
    ...
    if (includeTopConsumers) {
        output.topConsumers = report.topConsumers.map((e) => ({
            ...
            sizeHuman: (0, utils_1.formatBytes)(e.sizeBytes),
        }));
    }
    if (includeAlerts) {
        output.alerts = alerts.map((a) => ({
            ...
            severity: a.severity,
            message: a.message,
            timestamp: (0, utils_1.formatDate)(a.timestamp),
        }));
    }
...
// Prometheus Export
// ---------------------------------------------------------------------------
function exportPrometheus(report, opts) {
    ...
    if (opts?.includeDuplicates !== false) {
        ...
        for (const group of report.duplicateGroups) {
            lines.push(`${prefix}_duplicate_group_wasted_bytes{hash="${group.contentHash.slice(0, 16)}",count="${group.entries.length}"} ${group.wastedBytes}`);
        }
    }
    return lines.join('\n') + '\n';
}
...
// Slack Export
// ---------------------------------------------------------------------------
function exportSlack(report, alerts, opts) {
    const emoji = report.duplicateRatio > 0.2 ? '🔴' : report.duplicateRatio > 0.1 ? '🟡' : '🟢';
    const lines = [
        `${emoji} *Storage Guardian Report*`,
        `_${(0, utils_1.formatDate)(report.generatedAt)}_`,
        `Source: \`${report.source}\``,
        ``,
        `*Summary:*`,
        `• Total: ${(0, utils_1.formatBytes)(report.totalBytes)} across ${report.totalEntries} entries`,
        `• Unique: ${(0, utils_1.formatBytes)(report.uniqueBytes)}`,
        `• Wasted: ${(0, utils_1.formatBytes)(report.wastedBytes)} (${(report.duplicateRatio * 100).toFixed(1)}% duplicates)`,
    ...
    if (opts?.includeTopConsumers !== false && report.topConsumers.length > 0) {
        ...
        for (const e of report.topConsumers.slice(0, 5)) {
            lines.push(`• \`${e.name}\` — ${(0, utils_1.formatBytes)(e.sizeBytes)}`);
        }
    }
    return lines.join('\n');
}
...
// Markdown Export
// ---------------------------------------------------------------------------
function exportMarkdown(report, alerts, trend, opts) {
    const lines = [
        `# Storage Guardian Report`,
        ``,
        `**Generated:** ${(0, utils_1.formatDate)(report.generatedAt)}`,
        `**Source:** \`${report.source}\``,
        ...
        `| Total Size | ${(0, utils_1.formatBytes)(report.totalBytes)} |`,
        `| Unique Size | ${(0, utils_1.formatBytes)(report.uniqueBytes)} |`,
        `| Wasted (Duplicates) | ${(0, utils_1.formatBytes)(report.wastedBytes)} (${(report.duplicateRatio * 100).toFixed(1)}%) |`,
        `| Duplicate Groups | ${report.duplicateGroups.length} |`,
    ...
    if (report.duplicateGroups.length > 0 && opts?.includeDuplicates !== false) {
        ...
        for (const g of report.duplicateGroups.slice(0, 20)) {
            ...
            lines.push(`| \`${g.contentHash.slice(0, 16)}…\` | ${g.entries.length} (${files.slice(0, 80)}) | ${(0, utils_1.formatBytes)(g.wastedBytes)} |`);
        }
    }
    if (report.topConsumers.length > 0 && opts?.includeTopConsumers !== false) {
        ...
        for (const e of report.topConsumers) {
            lines.push(`| ${e.path ?? e.name} | ${(0, utils_1.formatBytes)(e.sizeBytes)} |`);
        }
    }
    const significantAlerts = alerts.filter((a) => a.severity === 'warning' || a.severity === 'critical');
    ...
    if (trend) {
        ...
        lines.push(`- Period: ${(0, utils_1.formatDate)(trend.period.from)} → ${(0, utils_1.formatDate)(trend.period.to)}`);
        ...
        lines.push(`- Storage growth: ${(0, utils_1.formatBytes)(trend.summary.storageGrowthBytes)} (${trend.summary.storageGrowthPercent.toFixed(1)}%)`);

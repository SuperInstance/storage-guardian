...
import { StorageGuardian } from './core/guardian';
import { FileSystemProvider } from './adapters/filesystem';
import { JsonFilePersistence, saveScan } from './persistence';
import { exportJson, exportPrometheus, exportSlack, exportMarkdown } from './export';
import { evaluateBuiltInRules } from './alerting';
import { analyzeTrend, compareReports } from './trend';
...
async function main() {
  ...
  const historyDir = (args.history as string) ?? '.storage-guardian-history';
...
async function scanCommand(args: Record<string, string | boolean>, historyDir: string) {
  const scanPath = (args._path as string) ?? '.';
  ...
  const provider = new FileSystemProvider({
    ...
    maxDepth: args['max-depth'] ? parseInt(args['max-depth'] as string) : undefined,
    excludePatterns: args.exclude
      ? (args.exclude as string).split(',').map((s) => s.trim())
      : undefined,
  });
  ...
  if (args.budget) {
    guardian.setBudget({ maxTotalBytes: parseInt(args.budget as string) });
  }
  ...
  const alerts = [
    ...guardian.getAlerts(),
    ...evaluateBuiltInRules(report, undefined, args.budget ? { maxTotalBytes: parseInt(args.budget as string) } : undefined),
  ];
  ...
  // Persist
  if (!noPersist) {
    const persistence = new JsonFilePersistence(historyDir);
    const record = await saveScan(report, alerts, persistence, resolvedPath);
  ...
  switch (format) {
    case 'json':
      output = exportJson(report, alerts);
      break;
    case 'prometheus':
      output = exportPrometheus(report);
      break;
    case 'slack':
      output = exportSlack(report, alerts);
      break;
    case 'markdown':
    default:
      output = exportMarkdown(report, alerts);
      break;
  }
...
async function trendCommand(args: Record<string, string | boolean>, historyDir: string) {
  const persistence = new JsonFilePersistence(historyDir);
...
async function compareCommand(args: Record<string, string | boolean>, historyDir: string) {
  ...
  console.log(compareReports(a.report, b.report));
}
